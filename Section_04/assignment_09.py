...
for line in fr:
   column = line.split('|')
   lookup_value = column[0]
   if d.get(lookup_value) is None:   #Can't find in the dictionary
       next_number = int(max(d.values())) + 1
       d[lookup_value] = str(next_number)         #It will append it in the dictionary
       column[0] = str(next_number)
       line_list.append("|".join(column))
   else:    #We aere able to find email in the dictionary
       column[0] = d.get(lookup_value)
       line_list.append("|".join(column))
